...
export const ExamCountdownCard: React.FC<ExamCountdownCardProps> = ({
  ...
  return (
    <div className="w-full max-w-2xl mx-auto font-pixel-heading select-none">
      ...
      <div 
        className={`border-2 border-[#222636] hover:border-[#383d52] transition-all rounded-xl p-4 sm:p-5 shadow-[0_10px_30px_rgba(0,0,0,0.8)] relative overflow-hidden bg-[#0a0b10]`}
      ...
      >
        ...
        {activeSlide === 1 && (
          <div className="relative z-10 animate-fadeIn space-y-4">
            ...
            {/* Simple Strava-Style Progress Chart */}
            <div className="pt-1">
              <div 
                className="relative bg-transparent transition-colors overflow-hidden group"
              >
                {/* Chart Scroll Container */}
                <div className="relative z-10 flex overflow-x-auto gap-4 sm:gap-5 items-end justify-start p-3 sm:p-4 min-h-[7rem] pb-8 [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-thumb]:bg-[#242630]/80 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full" onClick={() => onSelectGoalAsTask(goal.title || 'Target Goal')}>
                  {examDayDots.map((dot) => {
                    const minutes = dayLogs?.[dot.dateStr]?.totalMinutes || 0;
                    const isElapsed = dot.isElapsed;
                    const isToday = dot.isToday;

                    // Calculate max minutes for dynamic scaling (min 60 for scale)
                    const maxMinutesFound = Math.max(...examDayDots.map(d => dayLogs?.[d.dateStr]?.totalMinutes || 0), 60);
                    const percent = Math.min(100, (minutes / maxMinutesFound) * 100);
                    ...
                    return (
                      <div
                        ...
                        onMouseLeave={() => setHoveredDay(null)}
                        className={`relative flex flex-col items-center justify-end h-20 transition-transform duration-200 transform hover:scale-105 shrink-0 cursor-pointer ${
                          isToday ? 'z-10' : ''
                        }`}
                      >
                        <div className="relative flex flex-col items-center justify-end h-16 w-3 sm:w-4">
                           {minutes > 0 ? (
                             <div 
                               className={`w-[2px] transition-all duration-500 rounded-t-[1px] ${isToday ? 'bg-[#ff3b00] shadow-[0_0_8px_#ff3b00]' : 'bg-white'}`}
                               style={{ height: `${Math.max(2, percent)}%` }}
                             />
                           ) : (
                             <div className={`w-[2px] h-[2px] rounded-full ${isElapsed ? 'bg-zinc-700' : 'bg-zinc-900'}`} />
                           )}
                           
                           {isToday && (
                             <div 
                               className="absolute w-2 h-2 rounded-full bg-[#ff3b00] shadow-[0_0_8px_#ff3b00] -translate-x-1/2 left-1/2 transition-all duration-500" 
                               style={{ bottom: minutes > 0 ? `calc(${Math.max(2, percent)}% - 2px)` : '0px' }} 
                             />
                           )}
                        </div>
                        
                        {/* Rotated Date Label */}
                        <div className="absolute -bottom-2 sm:-bottom-3 translate-y-full origin-top transform -rotate-90">
                           <span className={`text-[6px] sm:text-[7px] font-pixel-label leading-none whitespace-nowrap z-10 ${isToday ? 'text-[#ff3b00] font-bold drop-shadow-[0_0_4px_rgba(255,59,0,0.4)]' : 'text-zinc-600'}`}>
              ...
              {/* Legend & Tooltip Bar */}
              <div className="mt-4 flex items-center justify-between text-[7.5px] font-pixel-label text-zinc-400 min-h-[16px]">
                {hoveredDay && hoveredDay.slide === 'exam' ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      <div className="w-[2px] h-3 bg-[#ff3b00] shadow-[0_0_4px_#ff3b00]" />
                      <div className="w-1.5 h-1.5 rounded-full bg-[#ff3b00] -ml-2.5" />
                      <span>TODAY</span>
                    </div>
                  </div>
                )}
